...
const MovieCards = ({ movie }) => {
  ...
  return (
    <Link href={`/movie/${movie.id}?${movie.title.toLowerCase().replace(" ", "_")}`}>
      <div
      ...
      >
        <div>
          <img src={`https://image.tmdb.org/t/p/w300${movie.poster_path}`} alt={movie.title} className="w-80% h-auto" />
        </div>
      </div>
    </Link>
  )
