import React from "react"
export default function Footer() {
  return (
    <div
      style={{ backgroundColor: "rgb(12, 45, 87)" }}
      className="m-4 flex justify-center rounded-lg"
    >
      <div className="container m-4 rounded-lg ">
        <h1 className="font-mono font-bold text-gray-400 text-2xl flex justify-center">
          Fallen Codes
        </h1>
        <div className="container mx-auto py-2">
          <div className="flex flex-wrap -mx-4">
            <div className="flex flex-col flex-grow justify-between items-center md:flex-row md:w-1/4 px-4 mb-4">
              <div className="bg-black rounded-lg shadow p-6 w-full">
                <h3 className="text-lg font-semibold mb-2 text-gray-400 flex justify-center">
                  Aween
                </h3>
                <div className="flex justify-center space-x-4">
                  <a
                    href="https://github.com/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-github mr-2"></i>
                  </a>
                  <a
                    href="https://www.linkedin.com/in/aween-ezzat-/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className=" hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-linkedin mr-2"></i>
                  </a>
                </div>
              </div>
            </div>
            <div className="flex flex-col flex-grow justify-between items-center md:flex-row md:w-1/4 px-4 mb-4">
              <div className="bg-black rounded-lg shadow p-6 w-full">
                <h3 className="text-lg font-semibold mb-2 text-gray-400 flex justify-center">
                  Shvan
                </h3>
                <div className="flex justify-center space-x-4">
                  <a
                    href="https://github.com/Sh89hkm"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-github mr-2"></i>
                  </a>
                  <a
                    href="https://www.linkedin.com/in/sh89hkm/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-linkedin mr-2"></i>
                  </a>
                </div>
              </div>
            </div>
            <div className="flex flex-col flex-grow justify-between items-center md:flex-row md:w-1/4 px-4 mb-4">
              <div className="bg-black rounded-lg shadow p-6 w-full">
                <h3 className="text-lg font-semibold mb-2 text-gray-400 flex justify-center">
                  Dilan
                </h3>
                <div className="flex justify-center space-x-4">
                  <a
                    href="https://github.com/Dilan-Ahmed"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="  hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-github mr-2"></i>
                  </a>
                  <a
                    href="https://www.linkedin.com/in/dilan-ahmed-02b671212/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-linkedin mr-2"></i>
                  </a>
                </div>
              </div>
            </div>
            <div className="flex flex-col flex-grow justify-between items-center md:flex-row md:w-1/4 px-4 mb-4">
              <div className="bg-black rounded-lg shadow p-6 w-full">
                <h3 className="text-lg font-semibold mb-2 text-gray-400 flex justify-center">
                  Astevan
                </h3>
                <div className="flex justify-center space-x-4">
                  <a
                    href="https://github.com/person4"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="  hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-github mr-2"></i>
                  </a>
                  <a
                    href="https://linkedin.com/in/person4"
                    target="_blank"
                    rel="noopener noreferrer"
                    className=" hover:underline flex items-center"
                    style={{ color: "rgb(252, 103, 54)" }}
                  >
                    <i className="fab fa-linkedin mr-2"></i>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
