...
export default function page({params}) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-4">
        ...
        <h2 className={`mb-3 text-2xl font-semibold`}>{formattedCategory(params.category)}</h2>
        <div className="flex flex-row justify-around m-2 p-2">
          <button className="m-2" onClick={handlePrevPage}>Pervious</button>
          <h1 className="m-2 mx-10"> {pageId} </h1>
          <button className="m-2" onClick={handleNextPage}>Next</button>
        </div>
